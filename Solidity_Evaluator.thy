...
fun dumpStorage::"StorageT \<Rightarrow> (Identifier \<times> Member) \<Rightarrow> String.literal \<Rightarrow> String.literal"
  where "dumpStorage s (i, Var x) = dump\<^sub>S\<^sub>t\<^sub>o\<^sub>r\<^sub>a\<^sub>g\<^sub>e s i i x"
  | "dumpStorage s (i, Method x) = id"

fun dumpProc::"(Address,StorageT) fmap \<Rightarrow> Data\<^sub>P \<Rightarrow> String.literal \<Rightarrow> String.literal"
  where "dumpProc st (ad, (xs, fb)) sl = (case fmlookup st ad of
          Some s \<Rightarrow> ad + (STR '': '') + foldr (dumpStorage s) xs sl + (STR ''\<newline>'')
        | None \<Rightarrow> sl + STR ''ERROR'')"
...
definition eval::"Gas \<Rightarrow> (S \<Rightarrow> Environment\<^sub>P \<Rightarrow> Environment \<Rightarrow> CalldataT \<Rightarrow> (unit, Ex, State) state_monad)
  ...
  where "eval g stmt\<^sub>e\<^sub>v\<^sub>a\<^sub>l stm addr adest aval acc d dat
        = (let (k,c,m,s,e) = load\<^sub>E\<^sub>n\<^sub>v\<^sub>i\<^sub>r\<^sub>o\<^sub>n\<^sub>m\<^sub>e\<^sub>n\<^sub>t (emptyStore, emptyStore, emptyStore, fmempty, emptyEnv addr adest aval) dat;
               ...
               s'        = fmupd addr s (foldr (\<lambda>x. (fmupd (fst x) fmempty)) acc fmempty);
           ...
           in (
             case (stmt\<^sub>e\<^sub>v\<^sub>a\<^sub>l stm e\<^sub>p e c z) of
              Normal ((), z') \<Rightarrow> (dump\<^sub>E\<^sub>n\<^sub>v\<^sub>i\<^sub>r\<^sub>o\<^sub>n\<^sub>m\<^sub>e\<^sub>n\<^sub>t (stack z') c (memory z') (the (fmlookup (storage z') addr)) e (map (\<lambda> (a,b,c). a) dat))
                             ...
                             + (foldr (dumpProc (storage z')) d (STR ''''))
...
lemma "eval 1000
            ...
            (STR '''')
            (STR ''0'')
            [(STR ''local'', STR ''100''), (STR ''myaddr'', STR ''100'')]
            [
              (STR ''myaddr'',
                ([(STR ''v1'', Var (STValue TBool)), (STR ''m1'', Method ([], (ASSIGN (Id (STR ''v1'')) FALSE), None))],
                SKIP))
            ]
            []
       = STR ''local.balance==100\<newline>myaddr.balance==100\<newline>myaddr: v1==false\<newline>\<newline>''"
  by (eval)
...
lemma "eval 1000
      ...
      = STR ''v1==5\<newline>myaddr.balance==100\<newline>myaddr: \<newline>''"
  by (eval)
...
lemma "eval 1000
            ...
            (STR '''')
            (STR ''0'')
            [(STR ''myaddr'', STR ''100''),(STR ''mya'', STR ''100'')]
            [
              (STR ''myaddr'',
       ...
       = STR ''v1==7\<newline>myaddr.balance==100\<newline>mya.balance==100\<newline>myaddr: \<newline>''"
  by (eval)
...
lemma "eval 1000
            ...
            (STR '''')
            (STR ''0'')
            [(STR ''myaddr'', STR ''100''), (STR ''extaddr'', STR ''100'')]
            [
              (STR ''extaddr'',
                ([(STR ''m1'', Method ([(STR ''v2'', Value (TSInt 8)), (STR ''v3'', Value (TSInt 8))], SKIP, Some (PLUS (LVAL (Id (STR ''v2''))) (LVAL (Id (STR ''v3''))))))],
                SKIP))
            ...
            [(STR ''v1'', (Value (TSInt 8), Stackint 0))]
       = STR ''v1==7\<newline>myaddr.balance==100\<newline>extaddr.balance==100\<newline>extaddr: \<newline>''"
  by (eval)
...
lemma "eval 1000
            ...
            (STR '''')
            (STR ''0'')
            [(STR ''089Be5381FcEa58aF334101414c04F993947C733'', STR ''100''), (STR ''myaddr'', STR ''100'')] 
            [
              (STR ''myaddr'',
                ([], SKIP))
            ]
            []
       = STR ''089Be5381FcEa58aF334101414c04F993947C733.balance==90\<newline>myaddr.balance==110\<newline>myaddr: \<newline>''"
  by (eval)

lemma "eval 1000
          stmt
          (TRANSFER (ADDRESS (STR ''Receiver'')) (UINT 256 10))
          (STR ''Sender'')
          (STR '''')
          (STR ''0'')
          [(STR ''Receiver'', STR ''100''), (STR ''Sender'', STR ''100'')]
          [
            (STR ''Receiver'',
          ...
          []
       = STR ''Receiver.balance==110\<newline>Sender.balance==90\<newline>Receiver: hello==true\<newline>\<newline>''"
  by (eval)
...
lemma "eval 1000
       ...
       = STR ''x==0\<newline>089Be5381FcEa58aF334101414c04F993947C733.balance==95\<newline>myaddr.balance==105\<newline>myaddr: \<newline>''"
  by (eval)
