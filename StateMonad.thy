...
fun
  option :: "'e \<Rightarrow> ('s \<Rightarrow> 'a option) \<Rightarrow> ('a \<Rightarrow> ('b, 'e, 's) state_monad) \<Rightarrow> ('b, 'e, 's) state_monad" where
 "option x f g = (\<lambda>s. (case f s of
    Some y \<Rightarrow> g y s
  | None \<Rightarrow> Exception x))"

notation (output)
  option  ("(option (_)//  (_)//  (_))" [1000,1000,1000] 1000)

lemma option_cong[fundef_cong]:
  assumes "b s = b' s"
    and "\<And>a. b' s = Some a \<Longrightarrow> m1 a s = m1' a s"
  shows "(option x b m1) s = (option x b' m1') s"
  by (simp add: assms(1) assms(2) option.case_eq_if)
...
lemma "option 0 (\<lambda>s. None) (\<lambda>x. return 2) = throw 0" by auto

lemma "option 0
        (\<lambda>s. Some 1)
        (\<lambda>x. return 2) = return 2" by auto
