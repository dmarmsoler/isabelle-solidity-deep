...
text \<open>
  ...
  The contract is defined by definition \<^emph>\<open>bank\<close> and consist of one state variable and two methods:
...
text \<open>
  We then verify that the following invariant (defined by \<^emph>\<open>BANK\<close>) is preserved by both methods:
...
value "eval 1000
          stmt
          (COMP
            (EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''deposit'') [] (UINT 256 10))
            (EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''withdraw'') [] (UINT 256 0)))
          (STR ''Attacker'')
          (STR '''')
          (STR ''0'')
          [(STR ''Bank'', STR ''100''), (STR ''Attacker'', STR ''100'')]
          [
            (STR ''Attacker'',
              [],
              ITE
...
abbreviation "BANK st \<equiv> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))"

definition frame_def: "frame bal st \<equiv> (\<exists>s. fmlookup (storage st) (STR ''Bank'') = Some s \<and> BANK st - SUMM s \<ge> bal \<and> bal \<ge> 0 \<and> POS s)"
...
assumes "Accounts.transfer ad adv v (accounts st) = Some acc"
    and "frame bal st"
    and "ad \<noteq> STR ''Bank''"
  shows "frame bal (st\<lparr>accounts := acc\<rparr>)"
proof (cases "adv = STR ''Bank''")
  case True
  define st' where "st' = st\<lparr>accounts := acc, stack := emptyStore, memory := emptyStore\<rparr>"
  from True assms(2) transfer_mono[OF assms(1)] have "(\<exists>s. fmlookup (storage st) (STR ''Bank'') = Some s \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0)" by (auto simp add:frame_def)
  then have "(\<exists>s. fmlookup (storage st') (STR ''Bank'') = Some s \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0)" by (auto simp add: st'_def)
  then show ?thesis using assms(2) by (auto simp add:st'_def frame_def)
next
  case False
  define st' where "st' = st\<lparr>accounts := acc, stack := emptyStore, memory := emptyStore\<rparr>"
  from False assms(2) assms(3) transfer_eq[OF assms(1)] have "(\<exists>s. fmlookup (storage st) (STR ''Bank'') = Some s \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0)" by (auto simp add:frame_def)
  then have "(\<exists>s. fmlookup (storage st') (STR ''Bank'') = Some s \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0)" by (auto simp add: st'_def)
  then show ?thesis using assms(2) by (auto simp add:st'_def frame_def)
qed 
...
lemma secureassign:
  assumes "stmt assign ep env cd st = Normal((), st')"
      and "fmlookup (storage st) (STR ''Bank'') = Some s"
      and "address env = (STR ''Bank'')"
      ...
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank'')) - (SUMM s) \<ge> bal"
      and "POS s"
    obtains s'
    where "fmlookup (storage st') (STR ''Bank'') = Some s'"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) - (SUMM s' + ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s)) \<ge> bal"
      and "accessStore x (stack st') = Some (KValue (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s))"
      and "POS s'"
proof -
  ...
  ultimately have ****:"st' = st''''\<lparr>storage := fmupd (STR ''Bank'') (fmupd ((sender env) + (STR ''.'' + STR ''balance'')) (ShowL\<^sub>i\<^sub>n\<^sub>t 0) s'') (storage st)\<rparr>" using c1 st''_def st'''_def st''''_def assms(1,3) by auto
  moreover define s' where s'_def: "s' = (fmupd ((sender env) + (STR ''.'' + STR ''balance'')) (ShowL\<^sub>i\<^sub>n\<^sub>t 0) s'')"
  ultimately have "fmlookup (storage st') (STR ''Bank'') = Some s'"
  ...
  proof (rule allI[OF allI])
    ...
    qed
  qed
  moreover have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))" using **** st''_def st'''_def st''''_def by simp
...
lemma securesender:
  assumes "expr SENDER ep env cd st = Normal((KValue v,t), st')"
      and "fmlookup (storage st) (STR ''Bank'') = Some s"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank'')) - SUMM s \<ge> bal \<and> POS s"
    obtains s' where
      "v = sender env" 
      and "t = Value TAddr"
      and "fmlookup (storage st') (STR ''Bank'') = Some s'"
...
proof -
  ...
  moreover obtain s'' where
      "fmlookup (storage st'') (STR ''Bank'') = Some s''"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'') (STR ''Bank'')) - SUMM s'' \<ge> bal \<and> POS s''"
...
lemma securelval:
  assumes "expr mylval ep env cd st = Normal((v,t), st')"
      and "fmlookup (denvalue env) (STR ''balance'') = Some (Storage (STMap TAddr (STValue (TUInt 256))), Storeloc STR ''balance'')"
      and "fmlookup (storage st) (STR ''Bank'') = Some s"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank'')) - SUMM s \<ge> bal \<and> bal \<ge> 0 \<and> POS s"
      and "address env = STR ''Bank''"
    obtains s' where "fmlookup (storage st') (STR ''Bank'') = Some s'"
      and "v = KValue (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s')"
      and "t = Value (TUInt 256)"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) - SUMM s' \<ge> bal \<and> bal \<ge> 0 \<and> POS s'"
proof -
  define st'' where "st'' = st\<lparr>gas := gas st - costs\<^sub>e mylval ep env cd st\<rparr>"
  with assms(3,4) have *: "fmlookup (storage st'') (STR ''Bank'') = Some s"
    and **: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'') (STR ''Bank'')) - SUMM s \<ge> bal \<and> POS s" by simp+
...
lemma plus_frame:
  assumes "expr (PLUS (LVAL (Ref (STR ''balance'') [SENDER])) VALUE) ep env cd st = Normal (kv, st')"
    ...
    shows "kv = (KValue (ShowL\<^sub>i\<^sub>n\<^sub>t (ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s) + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env))), Value (TUInt 256))"
      and "fmlookup (storage st') (STR ''Bank'') = Some s"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))"
proof -
  ...
  moreover from assms(4) st0_def st1_def st2_def st3_def `st' = st3` have "fmlookup (storage st') (STR ''Bank'') = Some s" by simp
  moreover from assms(5) st0_def st1_def st2_def st3_def `st' = st3` have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) - SUMM s \<ge> bal" by simp
  moreover have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))" using st0_def st1_def st2_def st3_def `st' = st3` by simp
  ultimately show "kv = (KValue (ShowL\<^sub>i\<^sub>n\<^sub>t (ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s) + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env))), Value (TUInt 256))"
   and "fmlookup (storage st') (STR ''Bank'') = Some s"
   and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))" by auto
qed
...
lemma deposit_frame:
  assumes "stmt deposit ep env cd st = Normal ((), st')"
      ...
      and "address env = (STR ''Bank'')"
      and "fmlookup (denvalue env) (STR ''balance'') = Some (Storage (STMap TAddr (STValue (TUInt 256))), Storeloc STR ''balance'')"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank'')) - SUMM s \<ge> bal + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env)"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s) + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env) < 2^256"
      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s) + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env) \<ge> 0"
      and "POS s"
    obtains s'
...
proof -
  ...
  moreover have "fmlookup (storage st0) (STR ''Bank'') = Some s" using st0_def assms(2) by simp
  moreover from assms(5) have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st0) (STR ''Bank'')) - SUMM s \<ge> bal + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue env)" using st0_def by simp
  ultimately have **: "kv = (KValue \<lfloor>(\<lceil>accessStorage (TUInt 256) (sender env + (STR ''.'' + STR ''balance'')) s\<rceil>::int) + \<lceil>svalue env\<rceil>\<rfloor>, Value (TUInt 256))"
    and st''_s:"fmlookup (storage st'') STR ''Bank'' = Some s"
    and ac: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st0) (STR ''Bank''))"
  ...
  moreover from st''_s st'''_def have s'''_s: "fmlookup (storage st''') (STR ''Bank'') = Some s" by simp
  ultimately have ****:"st' = st'''\<lparr>storage := fmupd (STR ''Bank'') (fmupd ((sender env) + (STR ''.'' + STR ''balance'')) (ShowL\<^sub>i\<^sub>n\<^sub>t v) s) (storage st''')\<rparr>"
  ...
  proof (rule allI[OF allI])
    ...
    qed
  qed
  moreover have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st') (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st) (STR ''Bank''))" using **** ac st0_def st'''_def by simp
  ultimately show ?thesis using that assms(5) by simp
qed

lemma secure:
        "address ev1 \<noteq> (STR ''Bank'') \<and> fmlookup ep1 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv1 st1' bal. frame bal st1 \<and> msel c1 t1 l1 xe1 ep1 ev1 cd1 st1 = Normal (rv1, st1') \<longrightarrow> frame bal st1')"
        "address ev2 \<noteq> (STR ''Bank'') \<and> fmlookup ep2 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv2 st2' bal. frame bal st2 \<and> ssel t2 l2 xe2 ep2 ev2 cd2 st2 = Normal (rv2, st2') \<longrightarrow> frame bal st2')"
        "address ev5 \<noteq> (STR ''Bank'') \<and> fmlookup ep5 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv3 st5' bal. frame bal st5 \<and> lexp l5 ep5 ev5 cd5 st5 = Normal (rv3, st5') \<longrightarrow> frame bal st5')"
        "address ev4 \<noteq> (STR ''Bank'') \<and> fmlookup ep4 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv4 st4' bal. frame bal st4 \<and> expr e4 ep4 ev4 cd4 st4 = Normal (rv4, st4') \<longrightarrow> frame bal st4')"
        "address lev \<noteq> (STR ''Bank'') \<and> fmlookup lep (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>ev cd ck mem st' bal. load lcp lis lxs lep lev0 lcd0 lck lmem lev lcd lst = Normal ((ev, cd, ck, mem), st') \<longrightarrow> (frame bal lst \<longrightarrow> frame bal st') \<and> address lev0 = address ev \<and> sender lev0 = sender ev \<and> svalue lev0 = svalue ev)"
        "address ev3 \<noteq> (STR ''Bank'') \<and> fmlookup ep3 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv3 st3' bal. frame bal st3 \<and> rexp l3 ep3 ev3 cd3 st3 = Normal (rv3, st3') \<longrightarrow> frame bal st3')"
        "(fmlookup ep6 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow>
          (\<forall>st6'. stmt s6 ep6 ev6 cd6 st6 = Normal((), st6') \<longrightarrow>
            ((address ev6 \<noteq> (STR ''Bank'') \<longrightarrow> (\<forall>bal. frame bal st6 \<longrightarrow> frame bal st6'))
           \<and> (address ev6 = (STR ''Bank'') \<longrightarrow>
              (\<forall>s val bal x. s6 = transfer
              \<and> fmlookup (storage st6) (STR ''Bank'') = Some s \<and> BANK st6 - (SUMM s + ReadL\<^sub>i\<^sub>n\<^sub>t val) \<ge> bal \<and> bal \<ge> 0 \<and> POS s
              ...
              \<longrightarrow> frame bal st6') \<and>
              (\<forall>s bal x. s6 = comp
              \<and> fmlookup (storage st6) (STR ''Bank'') = Some s \<and> BANK st6 - SUMM s \<ge> bal \<and> bal \<ge> 0 \<and> POS s
              \<and> fmlookup (denvalue ev6) (STR ''bal'') = Some (Value (TUInt 256), Stackloc x)
              ...
              \<and> fmlookup (denvalue ev6) (STR ''balance'') = Some (Storage (STMap TAddr (STValue (TUInt 256))), Storeloc STR ''balance'')
              \<and> sender ev6 \<noteq> address ev6
              \<longrightarrow> frame bal st6')
))))"
proof (induct rule: msel_ssel_lexp_expr_load_rexp_stmt.induct
[where ?P1.0="\<lambda>c1 t1 l1 xe1 ep1 ev1 cd1 st1. address ev1 \<noteq> (STR ''Bank'') \<and> fmlookup ep1 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv1 st1' bal. frame bal st1 \<and> msel c1 t1 l1 xe1 ep1 ev1 cd1 st1 = Normal (rv1, st1') \<longrightarrow> frame bal st1')"
   and ?P2.0="\<lambda>t2 l2 xe2 ep2 ev2 cd2 st2. address ev2 \<noteq> (STR ''Bank'') \<and> fmlookup ep2 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv2 st2' bal. frame bal st2 \<and> ssel t2 l2 xe2 ep2 ev2 cd2 st2 = Normal (rv2, st2') \<longrightarrow> frame bal st2')"
   and ?P3.0="\<lambda>l5 ep5 ev5 cd5 st5. address ev5 \<noteq> (STR ''Bank'') \<and> fmlookup ep5 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv5 st5' bal. frame bal st5 \<and> lexp l5 ep5 ev5 cd5 st5 = Normal (rv5, st5') \<longrightarrow> frame bal st5')"
   and ?P4.0="\<lambda>e4 ep4 ev4 cd4 st4. address ev4 \<noteq> (STR ''Bank'') \<and> fmlookup ep4 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv4 st4' bal. frame bal st4 \<and> expr e4 ep4 ev4 cd4 st4 = Normal (rv4, st4') \<longrightarrow> frame bal st4')"
   and ?P5.0="\<lambda>lcp lis lxs lep lev0 lcd0 lck lmem lev lcd lst. address lev \<noteq> (STR ''Bank'') \<and> fmlookup lep (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>ev cd ck mem st' bal. load lcp lis lxs lep lev0 lcd0 lck lmem lev lcd lst = Normal ((ev, cd, ck, mem), st') \<longrightarrow> (frame bal lst \<longrightarrow> frame bal st') \<and> address lev0 = address ev \<and> sender lev0 = sender ev \<and> svalue lev0 = svalue ev)"
   and ?P6.0="\<lambda>l3 ep3 ev3 cd3 st3. address ev3 \<noteq> (STR ''Bank'') \<and> fmlookup ep3 (STR ''Bank'') = Some (bank, SKIP) \<longrightarrow> (\<forall>rv3 st3' bal. frame bal st3 \<and> rexp l3 ep3 ev3 cd3 st3 = Normal (rv3, st3') \<longrightarrow> frame bal st3')"
  ...
  proof
    assume asm: "address env \<noteq> (STR ''Bank'') \<and> fmlookup e\<^sub>p (STR ''Bank'') = Some (bank, SKIP)"
    show ?RHS
...
next
  ...
  proof
    ...
    proof (rule allI[OF allI[OF allI[OF impI]]])
      ...
      moreover have x1: "\<forall>rv1' st1' bal. (frame bal st''') \<and> local.msel mm t ll (y # ys) e\<^sub>p env cd st''' = Normal (rv1', st1') \<longrightarrow> frame bal st1'" using 4(2)[OF ** _ _ _ _ _ *****] *** **** asm by auto
...
next
  ...
  proof
    ...
    proof (rule allI[OF allI[OF allI[OF impI]]])
      ...
      ultimately have "st' = st'''" by simp
      moreover have "\<forall>rv' st2' bal. (frame bal  st4') \<and> ssel t (hash loc v4) xs e\<^sub>p env cd st4' = Normal (rv', st2') \<longrightarrow> frame bal st2'" using 7(2)[OF **] *** asm by auto
...
next
  ...
  show ?case (is "_ \<longrightarrow> ?RHS")
  proof
    assume asm: "address e \<noteq> (STR ''Bank'') \<and> fmlookup e\<^sub>p (STR ''Bank'') = Some (bank, SKIP)"
    show ?RHS
    proof (rule allI[OF allI[OF allI[OF impI]]])
      fix rv1 and st' and bal
...
next
  ...
  proof
    ...
    proof (rule allI[OF allI[OF allI[OF impI]]])
      ...
      moreover from * obtain v st'' where **: "expr x e\<^sub>p e cd (st\<lparr>gas:=gas st - (costs\<^sub>e (NOT x) e\<^sub>p e cd st)\<rparr>) = Normal ((KValue v, Value TBool), st'')"
...
next
  ...
  show ?case using 27(1)[of "()" "st\<lparr>gas:=gas st - (costs\<^sub>e (LVAL i) e\<^sub>p e cd st)\<rparr>"] by (auto simp add:frame_def split:if_split_asm)
next
  ...
  proof
    ...
    proof (rule allI[OF allI[OF allI[OF impI]]])
      ...
      moreover from * ** *** obtain e'' cd' k\<^sub>l m\<^sub>l st'' where ****: "load False fp xe e\<^sub>p e' emptyStore emptyStore (memory (st\<lparr>gas := gas st - costs\<^sub>e (CALL i xe) e\<^sub>p e cd st\<rparr>)) e cd (st\<lparr>gas:=gas st - (costs\<^sub>e (CALL i xe) e\<^sub>p e cd st)\<rparr>) = Normal ((e'', cd', k\<^sub>l, m\<^sub>l), st'')"
        using e'_def by (auto split:if_split_asm result.split_asm)
      moreover from * **** have f1: "frame bal st''" and ad: "address e' = address e''"
        using asm 28(1)[OF _ ** _ *** _ _ _ _ e'_def, of _ "st\<lparr>gas := gas st - costs\<^sub>e (CALL i xe) e\<^sub>p e cd st\<rparr>" bla "(fp, f, Some x)" fp "(f, Some x)" f "Some x" x _ "st\<lparr>gas := gas st - costs\<^sub>e (CALL i xe) e\<^sub>p e cd st\<rparr>"] by (auto simp add:frame_def split:if_split_asm result.split_asm)
      ...
      moreover from * ** *** **** e'_def obtain st''' where *****: "stmt f e\<^sub>p e'' cd' (st''\<lparr>stack:=k\<^sub>l, memory:=m\<^sub>l\<rparr>) = Normal ((), st''')" by (auto split:if_split_asm result.split_asm)
      moreover from ad ad2 asm f1 ***** have f2:"frame bal st'''"
        using 28(2)[OF a1 ** _ *** _ _ _ _ e'_def _ ****, of bla "(fp, f, Some x)" "(f, Some x)" f "Some x" x e'' _ cd' _ k\<^sub>l m\<^sub>l _ st'', where s'd="st''\<lparr>stack := k\<^sub>l, memory := m\<^sub>l\<rparr>"] by (auto simp add:frame_def)
      ...
      ultimately have "st' = st''''\<lparr>stack:=stack st''\<rparr>" apply safe by auto
      moreover from f1 f2 ad ad2 asm a1 ***** ****** have "\<forall>rv4 st4' bal.
        frame bal st''' \<and>
        local.expr x e\<^sub>p e'' cd' st''' = Normal (rv4, st4') \<longrightarrow>
        frame bal st4'" using e'_def asm 28(3)[OF a1 ** _ *** _ _ _ _ e'_def _ **** _ _ _ _ _ *****, of bla "(fp, f, Some x)" " (f, Some x)" "Some x" x] apply safe by auto
...
next
  ...
  proof
    ...
    proof (rule allI[OF allI[OF allI[OF impI]]])
      ...
      moreover from * ** *** **** ***** obtain e'' cd' k\<^sub>l m\<^sub>l st'''' where *******: "load True fp xe e\<^sub>p e' emptyStore emptyStore emptyStore e cd st''' = Normal ((e'', cd', k\<^sub>l, m\<^sub>l), st'''')"
        using e'_def by (auto split:if_split_asm result.split_asm option.split_asm)
      moreover have "(\<forall>ev cda k m st' bal.
        local.load True fp xe e\<^sub>p e' emptyStore emptyStore emptyStore e cd st''' = Normal ((ev, cda, k, m), st') \<longrightarrow>
        (frame bal st''' \<longrightarrow> frame bal st') \<and> address e' = address ev \<and> sender e' = sender ev \<and> svalue e' = svalue ev)"
        using 29(3)[OF a1 ** _ _ _ _ *** _ **** _ _ _ _ ***** _ _ _ e'_def] asm ******* by simp
      then have "frame bal st'''' \<and> address e' = address e''" using ******* f2 f3 by blast
      ...
      moreover from f4 have f5: "frame bal (st''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" using transfer_frame[OF ******] asm by (simp add:frame_def)
      moreover from e'_def have ad2: "adv = address e'" using ffold_init_ad_same[of ct "(emptyEnv adv (address e) v)" "(fmdom ct)" e'] by simp
      moreover from * ** *** **** ***** ****** ******* ****** obtain st''''' where ********: "stmt f e\<^sub>p e'' cd' (st''''\<lparr>accounts := acc, stack:=k\<^sub>l, memory:=m\<^sub>l\<rparr>) = Normal ((), st''''')"
        using e'_def by (auto simp del: transfer.simps split:if_split_asm result.split_asm)
      moreover have "adv \<noteq> STR ''Bank''"
      proof (rule ccontr)
        assume "\<not> adv \<noteq> STR ''Bank''"
        with asm ** *** **** show False using bank_def fmap_of_list_SomeD[of "[(STR ''balance'', Var (STMap TAddr (STValue (TUInt 256)))), (STR ''deposit'', Method ([], deposit, None)), (STR ''withdraw'', Method ([], keep, None))]"] by auto
      qed
      with ad1 ad2 have ad: "address e'' \<noteq> STR ''Bank'' \<and> fmlookup e\<^sub>p (STR ''Bank'') = Some (bank, SKIP)" using asm by simp
      then have "(\<forall>bal. frame bal (st''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) \<longrightarrow> frame bal st''''')" using 29(4)[OF a1 ** _ _ _ _ *** _ **** _ _ _ _ ***** _ _ _ _ ******* _ _ _ ******] ******** e'_def by auto
      ...
      moreover from ad have "\<forall>rv4 st4' bal.
        ...
        local.expr x e\<^sub>p e'' cd' st''''' = Normal (rv4, st4') \<longrightarrow>
        frame bal st4'"
        using e'_def 29(5)[OF a1 ** _ _ _ _ *** _ **** _ _ _ _ ***** _ _ _ _ ******* _ _ _ ****** _ _ _ ********] by auto
...
next
  ...
  show ?case (is "_ \<longrightarrow> ?RHS")
  proof
    assume asm: "address e\<^sub>v \<noteq> (STR ''Bank'') \<and> fmlookup e\<^sub>p (STR ''Bank'') = Some (bank, SKIP)"
    show ?RHS
    proof (rule allI[OF allI[OF allI[OF allI[OF allI[OF allI[OF impI]]]]]])
      fix ev and cda and ck and mem and st'a and bal
      assume *: "local.load cp ((i\<^sub>p, t\<^sub>p) # pl) (e # el) e\<^sub>p e\<^sub>v' cd' sck' mem' e\<^sub>v cd st = Normal ((ev, cda, ck, mem), st'a)"
      moreover from * obtain v t st'' where **: "expr e e\<^sub>p e\<^sub>v cd st = Normal ((v,t),st'')" by (auto split: result.split_asm)
      moreover from * ** obtain cd'' e\<^sub>v'' m'' k'' where ***: "decl i\<^sub>p t\<^sub>p (Some (v,t)) cp cd (memory st'') (storage st'') (cd', mem', sck', e\<^sub>v') = Some (cd'', m'', k'', e\<^sub>v'')" by (auto split: option.split_asm)
      moreover from *** have ad: "address e\<^sub>v' = address e\<^sub>v'' \<and> sender e\<^sub>v' = sender e\<^sub>v'' \<and> svalue e\<^sub>v' = svalue e\<^sub>v''" using decl_address by simp
      moreover from * ** *** obtain ev' cda' ck' mem' st'a' where ****: "local.load cp pl el e\<^sub>p e\<^sub>v'' cd'' k'' m'' e\<^sub>v cd st''= Normal ((ev', cda', ck', mem'), st'a')" by (auto split: option.split_asm)
      ultimately have "ev = ev'" and "st'a = st'a'" by simp+
      
      from **** asm have IH: "(frame bal st'' \<longrightarrow> frame bal st'a') \<and> 
        address e\<^sub>v'' = address ev' \<and> sender e\<^sub>v'' = sender ev' \<and> svalue e\<^sub>v'' = svalue ev'" using 30(2)[OF ** _ ***] apply safe by (auto simp add:frame_def)
      show "(frame bal st \<longrightarrow> frame bal st'a) \<and> address e\<^sub>v' = address ev \<and> sender e\<^sub>v' = sender ev \<and> svalue e\<^sub>v' = svalue ev"
      proof
...
next
  ...
  proof
    assume *: "fmlookup e\<^sub>p STR ''Bank'' = Some (bank, SKIP)"
    ...
    proof
      ...
      proof
        ...
        proof
          show "?LHS"
          proof
            assume ad: "address e \<noteq> STR ''Bank''"
...
next
  ...
  show ?case (is "?LHS \<longrightarrow> ?RHS")
  proof
    assume 0: "fmlookup e\<^sub>p STR ''Bank'' = Some (bank, SKIP)"
    show ?RHS (is "\<forall>st6'. ?RHS st6'")
    proof
      fix st6'
      show "?RHS st6'" (is "?LHS \<longrightarrow> ?RHS")
      proof
        ...
        proof
          show "?LHS"
          proof
            assume asm: "address env \<noteq> STR ''Bank''"
            show "\<forall>bal. frame bal st \<longrightarrow> frame bal st6'"
            proof
              fix bal
              show "frame bal st \<longrightarrow> frame bal st6'"
                    ...
                    proof (cases x2)
                      ...
                      proof (cases rv)
                        ...
                        next
                          ...
                          proof (cases "accessStore l' (stack st'')")
                            case None
                            with * ** *** KCDptr Calldata MTArray LStackloc Memory show ?thesis by (simp split:if_split_asm)
                          next
                            case (Some sv)
                            then show ?thesis
                            proof (cases sv)
                              case (KValue x1)
                              with * ** *** KCDptr Calldata MTArray LStackloc Memory Some show ?thesis by (simp split:if_split_asm)
                            next
                              case c2: (KCDptr x2)
                              with * ** *** KCDptr Calldata MTArray LStackloc Memory Some show ?thesis by (simp split:if_split_asm)
                            next
                              ...
                              proof (cases "cpm2m p p' x t cd (memory st'')")
                                case None
                                with * ** *** KCDptr Calldata MTArray LStackloc Memory Some KMemptr show ?thesis by (simp split:if_split_asm)
                              next
                                case s3: (Some m')
                                with * ** *** KCDptr Calldata MTArray LStackloc Memory Some KMemptr have "st6' = st'' \<lparr>memory := m'\<rparr>" by (auto split:if_split_asm)
                                with f3 show ?thesis using asm by (simp add:frame_def)
                              qed
                            next
                              case (KStoptr p')
    ...
    proof
      ...
      proof
        ...
        proof
          show "?LHS"
          proof
            assume asm: "address e \<noteq> STR ''Bank''"
            show "\<forall>bal. frame bal st \<longrightarrow> frame bal st6'"
            proof
              fix bal
              show "frame bal st \<longrightarrow> frame bal st6'"
          ...
          proof
            ...
            next
              ...
              proof (rule allI[OF allI[OF allI]])
                ...
                show "?LHS s bal x" (is "?LHS \<longrightarrow> ?RHS")
                proof
                  assume ?LHS(is "?A1 \<and> ?A2 \<and> ?A3 \<and> ?A4 \<and> ?A5 \<and> ?A6 \<and> ?A7 \<and> ?A8 \<and> ?A9")
                  then have ?A1 and ?A2 and ?A3 and ?A4 and ?A5 and ?A6 and ?A7 and ?A8 and ?A9 by auto
                  ...
                  moreover from `?A2` have "fmlookup (storage (st\<lparr>gas := gas st - costs comp e\<^sub>p e cd st\<rparr>)) (STR ''Bank'') = Some s" by simp
                  moreover from `?A3` `?A4` have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st\<lparr>gas := gas st - costs comp e\<^sub>p e cd st\<rparr>)) (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0" by simp
                  moreover from `?A5` have "POS s" by simp
                  moreover from `?A8` have "accessStore x (stack (st\<lparr>gas := gas st - costs comp e\<^sub>p e cd st\<rparr>)) = Some (KValue (accessStorage (TUInt 256) (sender e + (STR ''.'' + STR ''balance'')) s))" by simp
                  ultimately obtain s'' where "fmlookup (storage st'') (STR ''Bank'') = Some s''"
                      and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'') (STR ''Bank'')) - (SUMM s'' + ReadL\<^sub>i\<^sub>n\<^sub>t (accessStorage (TUInt 256) (sender e + (STR ''.'' + STR ''balance'')) s)) \<ge> bal \<and> bal \<ge> 0"
                      and **: "accessStore x (stack st'') = Some (KValue (accessStorage (TUInt 256) (sender e + (STR ''.'' + STR ''balance'')) s))"
                      and "POS s''"
                    using secureassign[OF 00 _ ad `?A7`] that by blast
                  ...
                  ultimately show "frame bal st6'"
                    using 38(2)[OF x2 x1] `?A1` ad 0 `?A6` `?A9` by simp
                qed
              qed
            next
              show ?C (is "\<forall>s bal. ?LHS s bal")
            ...
            proof
              ...
              proof
                ...
                next
                  case False
                  show "frame bal st6'"
                  proof (cases "b = ShowL\<^sub>b\<^sub>o\<^sub>o\<^sub>l False")
                    case True
                    with False have "\<forall>st6' bal.
                    frame bal st' \<and>
                  ...
                  next
                    case f: False
                    with a1 * ** False show ?thesis by simp
                  qed
                qed
              qed
            qed
            ...
            proof
              ...
              proof
                ...
                moreover from * ** obtain fp f where ***: "fmlookup ct i = Some (Method (fp, f, None))"
                  by (auto split:if_split_asm option.split_asm Member.split_asm)
                moreover define e' where "e' = ffold_init ct (emptyEnv (address e) (sender e) (svalue e)) (fmdom ct)"
                moreover from * ** *** obtain e'' cd' k\<^sub>l m\<^sub>l st''' where ****: "load False fp xe e\<^sub>p e' emptyStore emptyStore (memory (st\<lparr>gas:=gas st - (costs (INVOKE i xe) e\<^sub>p e cd st)\<rparr>)) e cd (st\<lparr>gas:=gas st - (costs (INVOKE i xe) e\<^sub>p e cd st)\<rparr>) = Normal ((e'', cd', k\<^sub>l, m\<^sub>l), st''')"
                  using e'_def by (auto split:if_split_asm result.split_asm)
                moreover from * **** have f1: "frame bal st'''" and ad: "address e' = address e''"
                  using asm ff 0 41(1)[OF a1 ** _ *** _ _ _ _ e'_def, of bla "(fp, f, None)" fp "(f, None)" f None] by (auto simp add:frame_def)
                moreover from e'_def have ad2: "address e = address e'" using ffold_init_ad_same[of ct "(emptyEnv (address e) (sender e) (svalue e))" "(fmdom ct)" e'] by simp
                moreover from * ** *** **** e'_def obtain st'''' where *****: "stmt f e\<^sub>p e'' cd' (st'''\<lparr>stack:=k\<^sub>l, memory:=m\<^sub>l\<rparr>) = Normal ((), st'''')" by (auto split:if_split_asm result.split_asm)
                ultimately have "st' = st''''\<lparr>stack:=stack st'''\<rparr>" using * apply safe by simp
          ...
          proof
            ...
            proof
              ...
              proof
                ...
                then have f2: "frame bal st'''" by (simp add:frame_def)
                show "frame bal st'"
                proof (cases "fmlookup ct i")
                  ...
                  next
                    case False
                    moreover from f2 have "frame bal (st'''\<lparr>accounts := acc,stack:=emptyStore, memory:=emptyStore\<rparr>)" using transfer_frame[OF trans] asm by (simp add:frame_def)
                    then have "frame bal st''''" using f2 0 42(5)[OF a1 ** _ _ _ _ *** _ **** _ _ _ None trans] asm ***** False by (auto simp add:frame_def)
                ...
                next
                  ...
                  proof (cases "adv = STR ''Bank''")
                    case True
                    with 0 *** have ct_def: "ct = bank" by simp
  
                    moreover have
                      "(\<forall>(ev::Environment) cda ck mem st' bal.
                              local.load True fp xe e\<^sub>p e' emptyStore emptyStore emptyStore e cd st''' = Normal ((ev, cda, ck, mem), st') \<longrightarrow>
                              (frame bal st''' \<longrightarrow> frame bal st') \<and> address e' = address ev \<and> sender e' = sender ev \<and> svalue e' = svalue ev)"
                    ...
                    from ct_def ***** consider (withdraw) "i = STR ''withdraw''" | (deposit) "i = STR ''deposit''" using bank_def fmap_of_list_SomeD[of "[(STR ''balance'', Var (STMap TAddr (STValue (TUInt 256)))), (STR ''deposit'', Method ([], deposit, None)), (STR ''withdraw'', Method ([], keep, None))]"] by auto
                    then show ?thesis
                    proof cases
                      case withdraw
                      moreover have "fmlookup bank (STR ''withdraw'') = Some (Method ([], keep, None))" using bank_def by eval 
                      ultimately have "f=keep" and "fp = []" using *** ***** True 0 by auto
                      with ******** have *********: "stmt keep e\<^sub>p e'' cd' (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) = Normal ((), st'''''')" by simp
                      have "fmlookup (denvalue e'') STR ''balance'' = Some (Storage (STMap TAddr (STValue (TUInt 256))), Storeloc STR ''balance'')"
                      proof -
                        from bank_def have some: "fmlookup bank (STR ''balance'') = Some (Var (STMap TAddr (STValue (TUInt 256))))" by eval
                      ...
                      moreover from ad1 ad2 True have ad: "address e'' = STR ''Bank''" by simp
                      moreover from ad send1 send2 asm have "sender e'' \<noteq> address e''" by simp
                      moreover from f3 have f4: "frame bal (st'''''\<lparr>accounts := acc, stack:=k\<^sub>l, memory:=m\<^sub>l\<rparr>)" using transfer_frame[OF ******] asm by (simp add: frame_def)
                      then obtain s'''' where "fmlookup (storage (st'''''\<lparr>accounts := acc\<rparr>)) (STR ''Bank'') = Some s'''' \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc\<rparr>)) (STR ''Bank'')) - (SUMM s'''') \<ge> bal \<and> bal \<ge> 0 \<and> POS s''''" by (auto simp add:frame_def)
                      ultimately show ?thesis using 0 ******** `f=keep` 42(4)[OF a1 ** _ _ _ _ *** _ **** _ _ _ *****  _ _ _ _ e'_def ******* _ _ _ trans] by auto
                    next
                      case deposit
                      moreover have "fmlookup bank (STR ''deposit'') = Some (Method ([], deposit, None))" using bank_def by eval
                      ultimately have "f=deposit" and "fp = []" using *** ***** True 0 by auto
                      with ******** have *: "stmt deposit e\<^sub>p e'' cd' (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) = Normal ((), st'''''')" by simp
  
                      have f4: "frame (bal + ReadL\<^sub>i\<^sub>n\<^sub>t v) (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" and ad1: "address e' = address e''" and send1: "sender e' = sender e''" and sval1: "svalue e' = svalue e''"
                      proof -
                        have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st''''') (STR ''Bank'')) + ReadL\<^sub>i\<^sub>n\<^sub>t v" using transfer_add[OF ******] asm True by simp
                        moreover have "ReadL\<^sub>i\<^sub>n\<^sub>t v \<ge> 0" using transfer_val1[OF  ******] by simp
                        ultimately have "frame (bal + ReadL\<^sub>i\<^sub>n\<^sub>t v) (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" using f3 by (auto simp add:frame_def)
                        then show "frame (bal + ReadL\<^sub>i\<^sub>n\<^sub>t v) (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" and "address e' = address e''" and "sender e' = sender e''" and "svalue e' = svalue e''" using f2 0 42(3)[OF a1 ** _ _ _ _ *** _ **** _ _ _ ***** _ _ _ _ e'_def, of "KValue adv" "Value TAddr" TAddr fb "KValue v" "Value t"] asm ******* by (auto simp add:frame_def)
                      qed
                      moreover from sval1 sval2 have "v = svalue e''" by simp
                      ultimately have "frame (bal + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue e'')) (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" by simp
                      then obtain s''''' where II: "fmlookup (storage st''''') (STR ''Bank'') = Some s''''' \<and> BANK (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) - (SUMM s''''') \<ge> bal + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue e'') \<and> bal + ReadL\<^sub>i\<^sub>n\<^sub>t (svalue e'') \<ge> 0" and III:"POS s'''''" by (auto simp add:frame_def)
                      then have s'''''_def: "fmlookup (storage (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) STR ''Bank'' = Some s'''''" by simp
                      ...
                      from asm True have "address e \<noteq> (STR ''Bank'')" by simp
                      then have  "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st''''')  (STR ''Bank'')) + ReadL\<^sub>i\<^sub>n\<^sub>t v < 2^256" using transfer_val2[OF  ******] True by simp
                      moreover from `address e \<noteq> (STR ''Bank'')` have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc  (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st''''')  (STR ''Bank'')) + ReadL\<^sub>i\<^sub>n\<^sub>t v" using transfer_add[OF ******] True by simp
                      ultimately have abc: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>))  (STR ''Bank'')) < 2^256" by simp
  
                      from II have "fmlookup (storage (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank'') = Some s'''''  \<and> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank'')) - (SUMM s''''') \<ge> bal + ReadL\<^sub>i\<^sub>n\<^sub>t  (svalue e'') \<and> bal + ReadL\<^sub>i\<^sub>n\<^sub>t  (svalue e'') \<ge> 0" by (auto)
                      ...
                      proof (cases "fmlookup s''''' (sender e'' + (STR ''.'' + STR ''balance'')) = None")
                        ...
                        proof -
                          from II have "bal + \<lceil>svalue e''\<rceil> + SUMM s''''' \<le> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank''))" by simp
                          ...
                          ultimately have "bal + \<lceil>svalue e''\<rceil> \<le> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank''))" by simp
                      ...
                      next
                        ...
                        moreover from II have "bal + \<lceil>svalue e''\<rceil> + SUMM s''''' \<le> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank''))" by simp
                        ...
                        ultimately have "bal + \<lceil>svalue e''\<rceil> + ReadL\<^sub>i\<^sub>n\<^sub>t x \<le> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank''))" by simp
                        moreover from ff have "bal\<ge>0" by (auto simp add:frame_def)
                        ultimately have "\<lceil>svalue e''\<rceil> + ReadL\<^sub>i\<^sub>n\<^sub>t x \<le> ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) (STR ''Bank''))" by simp
                        ...
                        ultimately have "bal + \<lceil>svalue e''\<rceil> \<le> \<lceil>accessBalance (accounts (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)) STR ''Bank''\<rceil> - SUMM s'''''" and lck: "fmlookup s''''' (sender e'' + (STR ''.'' + STR ''balance'')) = Some x"  and "ReadL\<^sub>i\<^sub>n\<^sub>t x + \<lceil>svalue e''\<rceil> < 2 ^ 256" using ad1 ad2 True II x_def by simp+
                        ...
                        ultimately show ?thesis using Read_ShowL_id by simp
                      qed
                      moreover have "address e'' = STR ''Bank''" using ad1 ad2 True by simp
                      ultimately obtain s'''''' where "fmlookup (storage st'''''') (STR ''Bank'') = Some s''''''" and "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''''') (STR ''Bank'')) - SUMM s'''''' \<ge> bal" and "POS s''''''"
                      using deposit_frame[OF * s'''''_def _ yyy] III by auto
                      then show ?thesis using ff by (auto simp add:frame_def)
                    qed
                  next
                    case False
                    moreover from f2 have "frame bal st'''" using transfer_frame[OF ******] asm by simp
                    ...
                    then have f4: "frame bal (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>)" using transfer_frame[OF ******] asm by (simp add:frame_def)
                    
                    moreover from ad1 ad2 have "address e'' \<noteq> STR ''Bank'' \<and> fmlookup e\<^sub>p (STR ''Bank'') = Some (bank, SKIP)" using 0 False by simp
                    then have "\<forall>st6' bal.
                    frame bal (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) \<and>
                    local.stmt f e\<^sub>p e'' cd' (st'''''\<lparr>accounts := acc, stack := k\<^sub>l, memory := m\<^sub>l\<rparr>) = Normal ((), st6') \<longrightarrow>
                    frame bal st6'" using 42(4)[OF a1 ** _ _ _ _ *** _ **** _ _ _ ***** _ _ _ _ e'_def ******* _ _ _ ******] False asm by (auto simp add:frame_def)
              ...
              proof
                ...
                next
                  ...
                  proof (cases a)
                    ...
                    proof (cases "adv = STR ''Bank''")
                      case True
                      with 0 ** *** Some Pair have "f = SKIP" using bank_def by simp
                      with ****** have "st''''=  st'''
                    ...
                    next
                      ...
                      frame bal st6'" using asm Some Pair 43(3)[OF a1 ** _ _ _ *** _ _ _ _ ***** _ _ e'_def] using 0 by (simp add:frame_def)
                    ...
                    ultimately show "frame bal st'" by (simp_all add:frame_def)
                  qed
                qed
              qed
            qed
          ...
          proof
            ...
            proof (rule conj3)
              ...
              proof (rule allI[OF allI[OF allI[OF allI]]])
                ...
                show "?LHS s val bal x" (is "?LHS \<longrightarrow> ?RHS")
                proof
                  assume ?LHS (is "?A1 \<and> ?A2 \<and> ?A3 \<and> ?A4 \<and> ?A5 \<and> ?A6 \<and> ?A7 \<and> ?A8")
                  then have ?A1 and ?A2 and ?A3 and ?A4 and ?A5 and ?A6 and ?A7 and ?A8 by auto
                  ...
                  qed
                  with `?A6` `?A7` have 00:"expr (LVAL (Id (STR ''bal''))) e\<^sub>p e cd st'' = Normal ((KValue val, Value (TUInt 256)), st''')" using st'''_def st''_def by simp
                  moreover have "gas st'''>costs\<^sub>e SENDER e\<^sub>p e cd st'''"
                  proof (rule ccontr)
                    assume "\<not> costs\<^sub>e SENDER e\<^sub>p e cd st''' < gas st'''"
                    with c1 c2 show False using `?A1` `?A6` `?A7` * st''_def st'''_def by auto
                  qed      
                  ...
                  then obtain acc where ***:"Accounts.transfer (address e) (sender e) val (accounts st'''') = Some acc"
                    and ****: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (ReadL\<^sub>i\<^sub>n\<^sub>t val)"
                  proof -
                    from `?A1` * c1 00 ** obtain acc where acc_def: "Accounts.transfer (address e) (sender e) val (accounts st'''') = Some acc" using st''''_def st'''_def st''_def by (auto split: option.split_asm if_split_asm)
                    with ad obtain acc' where *: "subBalance (STR ''Bank'') val (accounts st'''') = Some acc'"
                      and "addBalance (sender e) val acc' = Some acc" by (simp split: option.split_asm)
                    moreover from * have "acc' = updateBalance(STR ''Bank'') (ShowL\<^sub>i\<^sub>n\<^sub>t (ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - ReadL\<^sub>i\<^sub>n\<^sub>t val)) (accounts st'''')" by (simp split: if_split_asm)
                    then have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc' (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (ReadL\<^sub>i\<^sub>n\<^sub>t val)" using Read_ShowL_id by simp
                    moreover from `?A8` ad have "sender e \<noteq> (STR ''Bank'')" by simp
                    ultimately have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) = ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (ReadL\<^sub>i\<^sub>n\<^sub>t val)" using addBalance_eq[of "sender e" val acc' acc " STR ''Bank''"] by simp
                  ...
                  proof (cases "fmlookup e\<^sub>p (sender e)")
                    ...
                    moreover from `?A2` have "fmlookup (storage st'''') (STR ''Bank'') = Some s" using st''_def st'''_def st''''_def by simp
                    moreover from `?A3` `?A4` have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (SUMM s + ReadL\<^sub>i\<^sub>n\<^sub>t val) \<ge> bal \<and> bal \<ge> 0" using st''_def st'''_def st''''_def by simp
                    with **** have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) - SUMM s \<ge> bal \<and> bal \<ge> 0" by simp
                    then have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st''''\<lparr>accounts := acc\<rparr>)) (STR ''Bank'')) - SUMM s \<ge> bal \<and> bal \<ge> 0" by simp
                    ultimately show ?thesis using `?A5` by (simp add:frame_def)
                  next
                    case (Some a)
                    then show ?thesis
                    proof (cases a)
                      ...
                      ultimately obtain st''''' where *****: "stmt f e\<^sub>p e' emptyStore (st''''\<lparr>accounts := acc, stack:=emptyStore, memory:=emptyStore\<rparr>) = Normal ((), st''''')" 
                        and ******: "st' = st'''''\<lparr>stack:=stack st'''', memory := memory st''''\<rparr>"  using `?A1` 00 ** *** Some * stmt.simps(8)[of SENDER "(LVAL (Id (STR ''bal'')))" e\<^sub>p e cd st] c1 st''_def st'''_def st''''_def by (auto split: if_split_asm result.split_asm)
                      ...
                      from *** have x11: "Accounts.transfer (address e) (sender e) val (accounts st'''') = Some acc" by simp
                      from Some Pair have x12: "fmlookup e\<^sub>p (sender e) = Some (ct,f)" by simp
                      have x13: "(ct, f) = (ct, f)" by simp
                      from e'_def have x14: "e' = ffold_init ct (emptyEnv (sender e) (address e) val) (fmdom ct)" by simp
                      have x15: "modify (\<lambda>st. st\<lparr>accounts := acc, stack := emptyStore, memory := emptyStore\<rparr>) st'''' = Normal ((), st''''\<lparr>accounts := acc, stack := emptyStore, memory := emptyStore\<rparr>)" by simp
                      
                      from `?A2` have "fmlookup (storage st'''') (STR ''Bank'') = Some s" using st''_def st'''_def st''''_def by simp
                      moreover from `?A3` `?A4` have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (SUMM s + ReadL\<^sub>i\<^sub>n\<^sub>t val) \<ge> bal \<and> bal \<ge> 0"  using st''_def st'''_def st''''_def by simp
                      with **** have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance acc (STR ''Bank'')) - SUMM s \<ge> bal \<and> bal \<ge> 0" by simp
                      then have "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts (st''''\<lparr>accounts := acc, stack:=emptyStore, memory:=emptyStore\<rparr>)) (STR ''Bank'')) - SUMM s \<ge> bal \<and> bal \<ge> 0" by simp
                      moreover from `?A8` ad have "sender e \<noteq> (STR ''Bank'')" by simp
                      with e'_def have "address e' \<noteq> STR ''Bank''" using ffold_init_ad_same[of ct "(emptyEnv (sender e) (address e) val)" "(fmdom ct)" e'] by simp
                      ultimately have "frame bal st'''''" using 0 ***** 43(3)[OF x1 x2 x3 x4 x5 x6 x7 x8 x9 x10 x11 x12 x13 x14 _ _ x15] `?A5` by (auto simp add:frame_def)
...
next
  ...
  proof
    ...
    proof
      ...
      proof
        ...
        proof
          show "?LHS"
          proof
            assume asm: "address e\<^sub>v \<noteq> STR ''Bank''"
            show "\<forall>bal. frame bal st \<longrightarrow> frame bal st6'"
            proof
              fix bal
              show "frame bal st \<longrightarrow> frame bal st6'"
              proof
                ...
                proof (cases ex)
                  case None
                  with * obtain cd' e' mem' sck' where **:"decl id0 tp None False cd (memory (st\<lparr>gas := gas st - costs (BLOCK ((id0, tp), ex) smt) e\<^sub>p e\<^sub>v cd st\<rparr>)) (storage (st\<lparr>gas := gas st - costs (BLOCK ((id0, tp), ex) smt) e\<^sub>p e\<^sub>v cd st\<rparr>)) (cd, memory (st\<lparr>gas := gas st - costs (BLOCK ((id0, tp), ex) smt) e\<^sub>p e\<^sub>v cd st\<rparr>), stack (st\<lparr>gas := gas st - costs (BLOCK ((id0, tp), ex) smt) e\<^sub>p e\<^sub>v cd st\<rparr>), e\<^sub>v) = Some (cd', mem', sck', e')" by (auto split:option.split_asm if_split_asm)
                  moreover from * None ** obtain st'' where ***: "stmt smt e\<^sub>p e' cd' (st\<lparr>gas := gas st - costs (BLOCK ((id0, tp), ex) smt) e\<^sub>p e\<^sub>v cd st, stack := sck', memory := mem'\<rparr>) = Normal ((), st'')" by (simp split:if_split_asm)
                  moreover from ** have ad: "address e' = address e\<^sub>v" using decl_address by simp
                  moreover from *** asm ad 0 f1 have "frame bal st''" using 44(3)[OF a1 None **] by (simp add:frame_def)
                ...
                next
                  ...
                  moreover from Some * ** obtain cd' e' mem' sck' where ***:"decl id0 tp (Some (v,t)) False cd (memory st') (storage st') (cd, memory st' , stack st', e\<^sub>v) = Some (cd', mem', sck', e')" by (auto split:option.split_asm if_split_asm)
                  moreover from ** *** have ad: "address e' = address e\<^sub>v" using decl_address by simp
                  moreover from * Some ** *** obtain st''' where ****: "stmt smt e\<^sub>p e' cd' (st'\<lparr>stack := sck', memory := mem'\<rparr>) = Normal ((), st''')" by (simp split:if_split_asm)
                  moreover from **** asm f2 ad have "frame bal st'''" using 44(2)[OF a1 Some ** _ ***] 0 by (simp add:frame_def)
        ...
        next
          show "?RHS" (is "?LHS \<longrightarrow> ?RHS")
          proof
            assume ad: "address e\<^sub>v = STR ''Bank''"
            show ?RHS (is "?A \<and> (?B \<and> ?C)")
            proof (rule conj3)
              show ?A (is "\<forall>s val bal x. ?LHS s val bal x")
              proof (rule allI[OF allI[OF allI[OF allI]]])
            ...
            next
              ...
              proof (rule allI[OF allI])
                ...
                show "?LHS s bal" (is "?LHS \<longrightarrow> ?RHS")
                proof
                  assume ?LHS (is "?A1 \<and> ?A2 \<and> ?A3 \<and> ?A4 \<and> ?A5 \<and> ?A6 \<and> ?A7")
                  then have ?A1 and ?A2 and ?A3 and ?A4 and ?A5 and ?A6 and ?A7 by auto

                  define st'' where "st'' = st\<lparr>gas := gas st - costs keep e\<^sub>p e\<^sub>v cd st\<rparr>"
                  with `?A2` `?A3` `?A4` `?A5` have 00: "fmlookup (storage st'') (STR ''Bank'') = Some s"
                      and **: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'') (STR ''Bank'')) - (SUMM s) \<ge> bal \<and> bal \<ge> 0 \<and> POS s" by simp+
                
                  from `?A1` * st''_def obtain v t st''' cd' e' sck' mem' st'''''
                    where ***: "expr mylval e\<^sub>p e\<^sub>v cd st'' = Normal ((v,t), st''')"
                      and ****: "decl (STR ''bal'') (Value (TUInt 256)) (Some (v, t)) False cd (memory st''') (storage st''') (cd, (memory st'''), (stack st'''), e\<^sub>v) = Some (cd', mem', sck', e')"
                      and *****: "stmt comp e\<^sub>p e' cd' (st'''\<lparr>stack := sck', memory := mem'\<rparr>) = Normal ((), st''''')"
                      and "st6' = st'''''"
                  by (auto split:if_split_asm option.split_asm result.split_asm)

                  define st'''' where "st'''' = st'''\<lparr>stack := sck', memory := mem'\<rparr>"
                
                  obtain s''' where
                      f1: "fmlookup (storage st''') (STR ''Bank'') = Some s'''"
                      and v_def: "v = KValue (accessStorage (TUInt 256) (sender e\<^sub>v + (STR ''.'' + STR ''balance'')) s''')"
                      and t_def: "t = Value (TUInt 256)"
                      and f2: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st''') (STR ''Bank'')) - (SUMM s''') \<ge> bal \<and> bal \<ge> 0 \<and> POS s'''"
                    using securelval[OF *** `?A6` 00 ** ad] by auto
                
                  with **** obtain s'''' where 
                    *******: "fmlookup (storage st'''') (STR ''Bank'') = Some s''''"
                    and bbal: "ReadL\<^sub>i\<^sub>n\<^sub>t (accessBalance (accounts st'''') (STR ''Bank'')) - (SUMM s'''') \<ge> bal \<and> bal \<ge> 0 \<and> POS s''''" using frame_def st''''_def by auto
                
                  from ad `?A7` have 
                    ad2: "address e' = STR ''Bank''"
                    and ss: "sender e'\<noteq>address e'" using decl_address[OF ****] by auto
                  ...
                  proof -
                    ...
                    with **** v_def t_def have "astack (STR ''bal'') (Value (TUInt 256)) (KValue (accessStorage (TUInt 256) (sender e\<^sub>v + (STR ''.'' + STR ''balance'')) s''')) (stack st''', e\<^sub>v) =  (sck',e')" by simp
                    with f1 v_def t_def have "sck' = push v (stack st''')" and "e' = updateEnv (STR ''bal'') t (Stackloc (ShowL\<^sub>n\<^sub>a\<^sub>t (toploc (stack st''')))) e\<^sub>v" by auto
                    ...
                    ultimately show ?thesis using t_def v_def `?A6` that st''''_def by simp
                  qed
                  with decl_address **** have sck: "accessStore x (stack st'''') = Some (KValue (accessStorage (TUInt 256) (sender e' + (STR ''.'' + STR ''balance'')) s''''))" by simp
                  ...
                  from `?A1` **** have a4: "decl id0 tp (Some (v, t)) False cd (memory st''') (storage st''') (cd, (memory st'''), (stack st'''), e\<^sub>v) = Some (cd', mem', sck', e')" by simp
                  from `?A1` ***** `st6' = st'''''` have a5: "local.stmt smt e\<^sub>p e' cd' st'''' = Normal ((), st6')" using st''''_def by simp
                  show "frame bal st6'" using 44(2)[OF a1 a2 a3 _ a4] 0 a5 ad2 `?A1` ******* bbal ****** lkup sck `?A4` ss st''''_def by auto
                qed
              qed
            qed
          qed
    ...
    qed
  qed
qed (simp_all add:frame_def)

corollary final1:
  assumes "fmlookup ep (STR ''Bank'') = Some (bank, SKIP)"
      and "stmt (EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''withdraw'') [] val) ep env cd st = Normal((), st')"
      and "address env \<noteq> (STR ''Bank'')"
      and "frame bal st"
      shows "frame bal st'"
  using assms secure(7)[of ep "(EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''withdraw'') [] val)" env cd st] by simp

corollary final2:
  assumes "fmlookup ep (STR ''Bank'') = Some (bank, SKIP)"
      and "stmt (EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''deposit'') [] val) ep env cd st = Normal((), st')"
      and "address env \<noteq> (STR ''Bank'')"
      and "frame bal st"
      shows "frame bal st'"
  using assms secure(7)[of ep "(EXTERNAL (ADDRESS (STR ''Bank'')) (STR ''deposit'') [] val)" env cd st] by simp
